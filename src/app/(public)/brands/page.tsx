          ...
          <div className="mt-10 sm:mt-12 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5">
            {brands.map(brand => (
              <Link key={brand.id} href={`/brands/${brand.slug}`}
                className="group flex flex-col items-center justify-between p-5 sm:p-6 bg-white rounded-2xl border border-slate-100 hover:border-blue-200 hover:shadow-lg transition-all duration-300 h-full min-h-[120px]">
                ...
                <div className="flex items-center justify-center h-14 w-full">
                  {brand.logo_url ? (
                    ...
                    <span className="text-base font-bold text-slate-700 group-hover:text-blue-700 text-center leading-tight">{brand.name}</span>
                  )}
                </div>
                <p className="mt-3 text-sm font-medium text-slate-500 group-hover:text-blue-600 flex items-center gap-1 transition-colors">
